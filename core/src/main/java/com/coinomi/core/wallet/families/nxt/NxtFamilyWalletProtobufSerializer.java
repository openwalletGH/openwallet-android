...
//    }
    ...
    public NxtFamilyWallet readWallet(Protos.WalletPocket walletProto, @Nullable KeyCrypter keyCrypter) throws UnreadableWalletException {
        ...
        NxtFamilyKey chain;
        if (keyCrypter != null) {
            chain = NxtFamilyKey.fromProtobuf(walletProto.getKeyList(), keyCrypter);
        } else {
            chain = NxtFamilyKey.fromProtobuf(walletProto.getKeyList());
        }

        NxtFamilyWallet pocket;
        if (walletProto.hasId()) {
            pocket = new NxtFamilyWallet(walletProto.getId(), chain, coinType);
        } else {
            pocket = new NxtFamilyWallet(chain, coinType);
        }
